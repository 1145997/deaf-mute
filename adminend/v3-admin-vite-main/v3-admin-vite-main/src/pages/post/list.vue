...
function getStatusAction(row: PostItem) {
  if (row.status === 1) {
    return {
      label: "下架",
      nextStatus: 3 as PostStatus,
      successMessage: "帖子已下架",
      confirmMessage: `确认下架帖子「${row.title}」吗？`,
  ...
  if (row.status === 2 || row.status === 3) {
    return {
      label: "发布",
      nextStatus: 1 as PostStatus,
      successMessage: "帖子已发布",
      confirmMessage: `确认将帖子「${row.title}」设为已发布吗？`,
...
async function handleUpdateStatus(row: PostItem) {
  ...
  try {
    await updatePostStatus(row.id, action.nextStatus)
  ...
  <div class="app-container">
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="tableData" border>
        ...
        <el-table-column prop="commentCount" label="评论" width="90" align="center" />
        ...
        <el-table-column label="操作" fixed="right" width="180" align="center">
          <template #default="{ row }">
            ...
            <el-button
              ...
              :type="getStatusAction(row)?.buttonType"
              link
    ...
    <el-drawer v-model="detailVisible" title="帖子详情" size="720px">
